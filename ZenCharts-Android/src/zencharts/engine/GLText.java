...
public class GLText {
	...
	public boolean load(String file, int size, int padX, int padY) {
		...
		gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MAG_FILTER, GL10.GL_LINEAR); // Set
																							// Magnification
																							// Filter
		gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_S, GL10.GL_CLAMP_TO_EDGE); // Set
																								// U
																								// Wrapping
		gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_T, GL10.GL_CLAMP_TO_EDGE); // Set
																								// V
																								// Wrapping
		...
		for (int c = 0; c < CHAR_CNT; c++) { // FOR Each Character (On Texture)
			charRgn[c] = new TextureRegion(textureSize, textureSize, x, y, cellWidth - 1, cellHeight - 1); // Create
																											// Region
																											// for
																											// Character
		...
		textureRgn = new TextureRegion(textureSize, textureSize, 0, 0, textureSize, textureSize); // Create
																									// Full
																									// Texture
																									// Region
	...
	/**
	 * Checks for collisions on label text.
	 * 
	 * @param inRect
	 * @return
	 */
	public ArrayList<RectF> collisionRects;

	private boolean checkCollision(RectF inRect)
	{

		boolean collision = false;
		int numRects = collisionRects.size();
		for (int i = 0; i < numRects; i++)
		{
			if (RectF.intersects(inRect, collisionRects.get(i)))
				collision = true;
		}

		if (!collision)
			collisionRects.add(inRect);
